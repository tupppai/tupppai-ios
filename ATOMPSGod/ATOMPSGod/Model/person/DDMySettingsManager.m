...
@implementation DDMySettingsManager

+ (NSURLSessionDataTask *)getPushSetting:(NSDictionary *)param withBlock:(void (^)(NSDictionary *, NSError *))block {
    return [[DDSessionManager shareHTTPSessionManager]GET:@"profile/get_push_settings" parameters:param success:^(NSURLSessionDataTask *task
, id responseObject) {
...
+ (NSURLSessionDataTask *)setPushSetting:(NSDictionary *)param withBlock:(void (^)(NSError *))block {
    return [[DDSessionManager shareHTTPSessionManager] POST:@"profile/set_push_settings" parameters:param success:^(NSURLSessionDataTask *task, id responseObject) {
        int ret = [(NSString*)[ responseObject objectForKey:@"ret"] intValue];
        if (ret == 1) {
            if (block) {
                block(nil);
