...
- (void)layoutSubviews {
    ...
    _bottomView.frame = CGRectMake(0, CGRectGetMaxY(_thinCenterView.frame), 0, 0);
    if (![_viewModel.totalPSNumber isEqualToString:@"0"]) {
...
+ (CGFloat)calculateCellHeightWith:(ATOMFollowPageViewModel *)viewModel {
    CGFloat imageViewHeight = viewModel.height;
    if (MAXHEIGHT<viewModel.height) {
        imageViewHeight = MAXHEIGHT;
    }
    if (viewModel.type == 1) {
        if ([viewModel.totalPSNumber isEqualToString:@"0"]) {
            return 60 + imageViewHeight + 60 + 8;
        } else {
            return 60 + imageViewHeight + 60 + 60 + 8;
        }
    } else {
        return 60 + imageViewHeight + 60 + 8;
    }
