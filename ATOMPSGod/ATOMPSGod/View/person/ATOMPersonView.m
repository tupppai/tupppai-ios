...
- (void)createSubView {
    ...
    [_userHeaderButton setBackgroundImageForState:UIControlStateNormal withURL:[NSURL URLWithString:[DDUserManager currentUser].avatar] placeholderImage:[UIImage imageNamed:@"head_portrait"]];
    ...
    NSString *str = [NSString stringWithFormat:@"%d", (int)[DDUserManager currentUser].fansNumber];
...
//    [fansLabelText addAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:kFont15], NSFontAttributeName, [UIColor colorWithHex:0xc3cbd2], NSForegroundColorAttributeName, nil] range:NSMakeRange(str.length + 1, 2)];
    ...
    NSMutableAttributedString *likeLabelText = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%d\n赞", (int)[DDUserManager currentUser].likeNumber] attributes:attributeDict];
    str = [NSString stringWithFormat:@"%d", (int)[DDUserManager currentUser].likeNumber];
